...
export default function CoordinatorDashboard() {
  ...
  const fetchDepartmentData = async () => {
    try {
      ...
      if (data.success) {
        ...
        if (data.department.timetableData) {
          ...
          if (data.department.timetableData.fileData) {
            ...
            const file = new File([byteArray], data.department.timetableData.fileName, {
            ...
            workbook.SheetNames.forEach(sheetName => {
              ...
              allSheets[sheetName] = XLSX.utils.sheet_to_json(sheet, { defval: '', raw: false })
            })
      ...
      if (configData.success && configData.university) {
        const config = configData.university.configuration || configData.university
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const facultyData = XLSX.utils.sheet_to_json(facultySheet, { defval: '' })
          const batchData = XLSX.utils.sheet_to_json(batchSheet, { defval: '' })
        ...
        try {
          ...
          for (let i = 0; i < jsonData.length; i++) {
            ...
            if (row && Array.isArray(row) && row.length >= 2) {
              ...
              if (typeof indexCol === 'string' && indexCol.toLowerCase().includes('index')) continue
              if (typeof slotCol === 'string' && /\d{1,2}:\d{2}/.test(slotCol)) {
  ...
  const getTYLockedSlotsByDay = () => {
    ...
    const lunchSlots = instituteConfig.lunchSlots?.TY && Array.isArray(instituteConfig.lunchSlots.TY)
  ...
  const getBTechLockedSlotsByDay = () => {
    ...
    const lunchSlots = instituteConfig.lunchSlots?.BTech && Array.isArray(instituteConfig.lunchSlots.BTech)
    ...
    for (const [key, value] of Object.entries(resultsObj || {})) {
      try {
        ...
        if (!Array.isArray(excelData)) {
          excelData = excelData && typeof excelData === 'object' ? [excelData] : []
        }
  ...
  const handlePECConfirm = async () => {
    ...
    try {
      ...
      if (instituteConfig?.lunchSlots) {
        formData.append('lunchSlots', JSON.stringify(instituteConfig.lunchSlots))
      }
      ...
      formData.append('btechPecTheorySlots', JSON.stringify(btechPecTheorySlots))
      formData.append('btechPecLabSlots', JSON.stringify(btechPecLabSlots))
      formData.append('onlineConfig', JSON.stringify(onlineConfig))
      formData.append('facultyAvailability', JSON.stringify(facultyAvailability))
                  ...
                  <>
                    ...
                    {isStandardDept && configStep === 3 && (
                      <div className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl border border-blue-200 p-4">
                        <h3 className="text-sm font-bold text-blue-600 mb-3">TY PEC Configuration</h3>
                        <PECSlotConfig
                          timeSlots={timeSlots}
                    ...
                    {isStandardDept && configStep === 4 && (
                      <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl border border-purple-200 p-4">
                        <h3 className="text-sm font-bold text-purple-600 mb-3">BTech PEC Configuration</h3>
                        <PECSlotConfig
                          timeSlots={timeSlots}
