      ...
      <div className="flex items-start justify-between relative">
        {steps.map((step, idx) => (
          <div key={step.id} className="flex flex-col items-center flex-1 relative z-10">
            ...
            <p className={`text-xs font-semibold mt-2 text-center ${currentStep >= step.id ? 'text-gray-800' : 'text-gray-400'}`}>
              {step.label}
            </p>
            {idx < steps.length - 1 && (
              <div className="absolute left-1/2 right-0 top-6 h-1 -z-10" style={{ width: 'calc(100% - 24px)', left: 'calc(50% + 24px)' }}>
