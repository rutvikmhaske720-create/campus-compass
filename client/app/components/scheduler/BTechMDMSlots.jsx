...
export default function BTechMDMSlots({ onSave, initialData, lockedSlots, extractedData }) {
  const [mdmData, setMdmData] = useState(initialData || {})
  const SLOTS = getSlots(extractedData)

  const getLockedSlots = () => {
    const locked = []
    if (lockedSlots?.BTech) {
      locked.push(...lockedSlots.BTech)
    }
    return locked
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-secondary p-4">
      <div className="flex items-center space-x-2 mb-4">
        ...
        <div>
          <h3 className="text-base font-bold text-gray-800">B.Tech (Fourth Year) - MDM Slots</h3>
